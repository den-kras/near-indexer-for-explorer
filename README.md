...
- AWS S3 (permission to read from buckets):
    ...
    Ensure you have [AWS Credentials configured](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html) to access AWS S3:
  ...
  > ```
  > ```bash
  > $ echo "AWS_SECRET_ACCESS_KEY=<your_aws_secret_access_key_id>" > .env
  > ```
